 ...
 * 
 */
public class HeaderView extends FlowPanel implements Model.DataObserver, Surface.Observer {
	 ...
	 * author.
	 */
	private class EditController implements BlurHandler, KeyPressHandler, ClickHandler {

		/**
		 * A view displayed after a user submitted an author's email address,
		 * but before the {@link Model} responds with success or failure. If the
		 * server reports a failure, the {@link PendingAuthorView} is also used
		 * to display that error.
		 */
		private class PendingAuthorView extends SimplePanel implements ClickHandler, Model.SuccessCallback {
			 ...
			 * 
			 * @param name
			 *            the name to display while the model saves the change
			 *            on the server
			 */
			...
			private void showError(String message) {
				final SpanElement errorElement = getElement().appendChild(Document.get().createSpanElement());
		...
		public EditController(Model model, Images images, String styleName) {
			this.model = model;
			button = Buttons.createPushButtonWithImageStates(images.headerAddAuthorButtonUp().createImage(), images.headerAddAuthorButtonHv().createImage(), "header-add-author-button", this);
		...
		public void onBlur(BlurEvent event) {
			// Unintended blur events can happen as visibility is toggled.
			// editMode
			// ensures that we only handle blur when we are editing. Setting a
			// boolean
		...
		private PendingAuthorView createPendingAuthorView(String name) {
			final Element parentElement = textBox.getElement().getParentElement().cast();
			final Element element = parentElement.insertBefore(Document.get().createSpanElement(), textBox.getElement()).cast();
		...
		private void removePendingAuthorView(PendingAuthorView view) {
			final Element pendingElement = view.getElement().getParentElement().cast();
	...
	/**
	 * Encapsulates the views and behaviors associated with displaying and
	 * closing the {@link SurfaceListView} and keeping the associated
	 * {@link ToggleButton} in sync.
	 */
	private static class SurfaceListViewController implements ClickHandler, SurfaceListView.Observer {
		...
		/**
		 * @param model
		 *            the model to which the enclosed {@link SurfaceListView}
		 *            will communicate.
		 * @param images
		 ...
		 */
		public SurfaceListViewController(Model model, Images images, String styleName) {
			button = Buttons.createToggleButtonWithImageStates(images.headerSurfacesButtonUp().createImage(), images.headerSurfacesButtonHv().createImage(), images.headerSurfacesButtonDn()
					.createImage(), styleName, this);
	...
	private final SpanElement surfaceNameElement = Document.get().createSpanElement();

	private final SpanElement authorNamesElement = Document.get().createSpanElement();
	...
	public HeaderView(Images images, RootPanel parent, final Model model) {
		...
		final SurfaceListViewController controller = new SurfaceListViewController(model, images, "spc-button");
		parent.add(controller.getSurfaceListView());

		final Element elem = getElement();
		elem.setId("header");

		add(Buttons.createPushButtonWithImageStates(images.headerAddButtonUp().createImage(), images.headerAddButtonHv().createImage(), images.headerAddButtonDn().createImage(), "add-button",
				new ClickHandler() {
					@Override
					public void onClick(ClickEvent event) {
						model.createNote(NOTE_DEFAULT_X, NOTE_DEFAULT_Y, NOTE_DEFAULT_WIDTH, NOTE_DEFAULT_HEIGHT);
					}
				}));
	...
	private void attachTitleView(Images images, Surface surface) {
		final Element titleElement = getElement().appendChild(Document.get().createDivElement()).cast();
