...
public class ServiceImpl extends RemoteServiceServlet implements Service {
	...
	private static Surface toClientSurface(Store.Surface surface) {
		...
		return new Surface(KeyFactory.keyToString(surface.getKey()), surface.getTitle(), names.toArray(new String[names.size()]), surface.getNotes().size(), surface.getLastUpdatedAt());
	}
	...
	@Override
	public AddAuthorToSurfaceResult addAuthorToSurface(final String surfaceKey, final String email) throws AccessDeniedException {
	...
	@Override
	public Date changeNoteContent(final String noteKey, final String content) throws AccessDeniedException {
		final User user = tryGetCurrentUser(UserServiceFactory.getUserService());
		final Store.Api api = store.getApi();
		try {
	...
	@Override
	public Date changeNotePosition(final String noteKey, final int x, final int y, final int width, final int height) throws AccessDeniedException {
		final User user = tryGetCurrentUser(UserServiceFactory.getUserService());
		final Store.Api api = store.getApi();
		try {
	...
	@Override
	public CreateObjectResult createNote(final String surfaceKey, final int x, final int y, final int width, final int height) throws AccessDeniedException {
		final User user = tryGetCurrentUser(UserServiceFactory.getUserService());
		final Store.Api api = store.getApi();
		try {
			...
			final CreateObjectResult result = new CreateObjectResult(KeyFactory.keyToString(note.getKey()), note.getLastUpdatedAt());
			tx.commit();
	...
	public UserInfoResult getUserInfo() throws AccessDeniedException {
		...
		try {
			...
			final UserInfoResult result = new Service.UserInfoResult(new Author(user.getEmail(), user.getNickname()), getSurface(api, surfaceKey), userService.createLogoutURL(userService
					.createLoginURL("/")));
			return result;
		} finally {
			api.close();
