...
public class FriendListPresenter implements Presenter {
  ...
  public FriendListPresenter(FriendsServiceAsync rpcService,
      MessagesServiceAsync messagesService, SimpleEventBus eventBus,
      Display view) {
  ...
  public void sortFriendSummaryDTO() {
    for (int i = 0; i < friendSummaries.size(); ++i) {
      for (int j = 0; j < friendSummaries.size() - 1; ++j) {
        if (friendSummaries.get(j).getDisplayName()
            .compareToIgnoreCase(
                friendSummaries.get(j + 1).getDisplayName()) >= 0) {
  ...
  private void fetchFriendSummaryDTO() {
    ...
    new RPCCall<ArrayList<FriendSummaryDTO>>() {
      ...
      protected void callService(
          AsyncCallback<ArrayList<FriendSummaryDTO>> cb) {
      ...
      public void onFailure(Throwable caught) {
        Window.alert("Error fetching friend summaries: "
            + caught.getMessage());
      }
    }.retry(3);
  ...
  private void fireFriendListChangeEvent() {
    ...
    if (selection.isEmpty())
      eventBus.fireEvent(new FriendListChangedEvent(friendSummaries));
    else
      eventBus.fireEvent(new FriendListChangedEvent(selection));
  }
